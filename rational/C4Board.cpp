...
ostream &operator<<( ostream &output, C4Board &B){
  for(int i=1;i<=B.numCols;i++){//Shows column number column 
	if(i<10) output << "  " << i << " ";
	else output << " " << i << " ";
  }
  output << endl;
  
  int j=B.Board[0].getMaxDiscs()-1;
  for(j; j>=0; j--){	//prints last element of the column first
    output << "| ";
    for(int i=0; i<B.numCols; i++){				//from row 0-numCols
	output << B.Board[i].getDisc(j) << " | ";		//print each column
    } output << endl;					//new line after column
  } output << endl;
  return output;
}
...
void C4Board::play(int players){
  ...
  do{ 
    ...
    if(!Board[rowInput].isFull()) Board[rowInput]+=add;	//Places letter
    else {cout << "This is an invalid row" << endl; continue; }	//makes sure the row isn't full
    cout << (*this);				//Displays board
